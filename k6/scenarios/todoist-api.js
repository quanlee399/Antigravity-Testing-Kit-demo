...
export function todoistScenario() {
  ...
  group('Todoist_01_GetProjects', () => {
    ...
    check(res, {
      'Todoist Get Projects status is 200 or 401': (r) => r.status === 200 || r.status === 401,
    });
    randomThinkTime(1, 2);
  });
  ...
  group('Todoist_02_GetTasks', () => {
    ...
    check(res, {
      'Todoist Get Tasks status is 200 or 401': (r) => r.status === 200 || r.status === 401,
    });
    randomThinkTime(1, 2);
  });
  ...
  group('Todoist_03_CreateTask', () => {
    ...
    check(res, {
      'Todoist Create Task status is 200, 201 or 401': (r) => r.status === 200 || r.status === 201 || r.status === 401,
    });
    randomThinkTime(1, 2);
  });
}
